...
const QuestionDetail = ({ questionId }: any) => {
  ...
  useEffect(() => {
    const fetchuser = async () => {
      try {
        ...
        const isBookmarked = currentUser?.saves?.includes(matchedquestion._id) || false;
        setquestion({ ...matchedquestion, isBookmarked });
      } catch (error) {
        console.log(error);
      } finally {
        setloading(false);
  ...
  const handlebookmark = async () => {
    ...
    try {
      const res = await axiosInstance.patch(`/user/save/${user._id}`, {
        questionId: question._id,
      });
      const isSaved = res.data.saved;
      const updatedUser = res.data.data;
      const currentUser = JSON.parse(localStorage.getItem("user") || "{}");
      localStorage.setItem("user", JSON.stringify({...updatedUser, token: currentUser.token}));
      setquestion((prev: any) => ({ ...prev, isBookmarked: isSaved }));
